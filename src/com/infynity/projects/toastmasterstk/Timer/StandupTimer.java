...
public class StandupTimer extends Activity implements OnClickListener {
    ...
    private int redWarningTime = 0;
    private int yellowWarningTime = 0;
    private int greenWarningTime = 0;
    ...
    private void initializeTimerValues() {
        int meetingLength = getIntent().getIntExtra("meetingLength", 5);
        int numParticipants = getIntent().getIntExtra("numParticipants", 1);
        redWarningTime = getIntent().getIntExtra("redWarningTime", (1*60))/numParticipants;
        yellowWarningTime = getIntent().getIntExtra("yellowWarningTime", (3*60))/numParticipants;
        greenWarningTime = getIntent().getIntExtra("greenWarningTime", (4*60))/numParticipants;
        
        Logger.d("Data from Intent: meetingLength = " + meetingLength);
        Logger.d("Data from Intent: numParticipants = " + numParticipants);
        Logger.d("Data from Intent: greenWarningTime = " + greenWarningTime);
        Logger.d("Data from Intent: yellowWarningTime = " + yellowWarningTime);
        Logger.d("Data from Intent: redWarningTime = " + redWarningTime);
    ...
    protected synchronized void updateDisplay() {
        if (individualStatusInProgress()) {
            ...
            individualTimeRemaining.setTextColor(TimeFormatHelper.determineColor(remainingIndividualSeconds, greenWarningTime, yellowWarningTime, redWarningTime));
        ...
        totalTimeRemaining.setTextColor(TimeFormatHelper.determineColor(remainingMeetingSeconds, greenWarningTime, yellowWarningTime, redWarningTime));
    }
    ...
    protected synchronized void updateTimerValues() {
        ...
        if (remainingIndividualSeconds > 0) {
            ...
            if (remainingIndividualSeconds == redWarningTime) {
